...
export function PartidoCard({
  ...
  const pronosticoCerrado = isPredictionClosed(partido.fecha);
  const countdownLabel =
    allowPronostico && !tieneResultadoFinal
      ? getPredictionCountdownLabel(partido.fecha, 60, now)
      : null;
        ...
        <div className={compact ? "px-3 md:px-4" : "p-4 md:p-5"}>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 lg:justify-end">
            ...
            {miPronostico && (
              ...
              >
                Mi pronóstico: {miPronostico.golesLocal} - {miPronostico.golesVisitante}
              </Badge>
            )}
            ...
            {allowPronostico && !tieneResultadoFinal && (
              <Button
                ...
                className="h-8 rounded-xl bg-[#39A935] px-3 text-xs text-white hover:bg-[#247A28] disabled:bg-slate-300"
              >
                {miPronostico ? "Editar pronóstico" : "Pronosticar"}
              </Button>
            )}
...
function TeamSlot({
  ...
  return (
    ...
    >
      ...
      <TeamFlag bandera={team.bandera} codigo={team.codigo} nombre={team.nombre} />
...
function TeamFlag({
  ...
  const flagClassName =
    "h-9 w-12 shrink-0 object-contain shadow-none";
