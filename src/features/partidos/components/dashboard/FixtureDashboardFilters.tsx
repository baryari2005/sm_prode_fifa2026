...
type FixtureDashboardFiltersProps = {
  busqueda: string;
  faseActiva: FixturePhaseSlug | null;
  ...
  onPhaseChange: (fase: FixturePhaseSlug) => void;
  onGrupoChange: (grupo: string) => void;
  onShowOnlyPendingChange: (value: boolean) => void;
};

export function FixtureDashboardFilters({
  busqueda,
  faseActiva,
  faseActivaLabel,
  mostrandoFaseGrupos,
      ...
      <div className="relative z-10 space-y-5">
          ...
          <div className="flex flex-col gap-3 xl:min-w-[360px] xl:items-end">
            <div className="flex w-full flex-col gap-3 sm:flex-row xl:justify-end">
              <label className="flex items-center gap-3 text-sm font-semibold text-white/72">
                ...
                />
                <span>{showOnlyPending ? "Solo pendientes" : "Ver todos"}</span>
              </label>
        ...
        <div className="overflow-x-auto pb-1">
          <div className="flex min-w-max gap-2">
            {FIXTURE_PHASE_OPTIONS.map((fase) => {
              const active = (faseActiva ?? "grupos") === fase.slug;

              return (
                <Button
                  key={fase.slug}
                  type="button"
                  onClick={() => onPhaseChange(fase.slug)}
        ...
        {mostrandoFaseGrupos && gruposDisponibles.length > 0 ? (
          <Tabs value={grupoSeleccionado ?? gruposDisponibles[0]} className="space-y-0">
            <div className="overflow-x-auto pb-1">
