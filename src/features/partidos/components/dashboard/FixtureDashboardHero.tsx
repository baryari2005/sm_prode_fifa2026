...
type FixtureDashboardHeroProps = {
  totalPartidos: number;
  faseActivaLabel: string;
  gruposVisibles: number;
...
export function FixtureDashboardHero({
  totalPartidos,
  faseActivaLabel,
  gruposVisibles,
  hasLiveMatches,
      ...
      <div className="grid w-full min-w-0 gap-4 2xl:grid-cols-[minmax(0,2.05fr)_minmax(300px,0.95fr)] 2xl:items-stretch">
        <section className="relative h-full w-full min-w-0 overflow-hidden rounded-[30px] border border-white/10 bg-[#1E2C46] px-4 py-5 text-white shadow-[0_24px_70px_rgba(2,6,23,0.24)] md:px-6 md:py-6 xl:h-[364px] xl:px-7 xl:py-6 2xl:h-[420px] 2xl:px-8 2xl:py-7">
          ...
          <div className="relative z-10 flex h-full max-w-[62%] min-w-0 flex-col">
            ...
            <div className="flex flex-wrap gap-3 pt-8 xl:pt-10 2xl:pt-18">
              <LiveRefreshBadge
                isRefreshing={isAutoRefreshing}
                nextRefreshIn={nextAutoRefreshIn}
                lastRefreshAt={lastAutoRefreshAt}
                shortText
                className="rounded-2xl border border-white/15 bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/10"
              />
            </div>
          </div>
          ...
          <div className="pointer-events-none absolute bottom-[-18px] right-[-6px] z-20 hidden h-[400px] w-[389px] xl:block 2xl:bottom-[-22px] 2xl:right-[-2px] 2xl:h-[464px] 2xl:w-[454px]">
            ...
            <HeroVisualImage
              src={brandImages.mascots.fixture}
