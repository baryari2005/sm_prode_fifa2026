...
import { Network, ShieldCheck, ShieldPlus, TimerReset } from "lucide-react";
...
import { ReglasCruceList } from "./ReglasCruceList";
import { LateralSummaryHeader } from "@/components/ui/lateralSummaryHeader";

type ReglasCrucesOverviewProps = {
  reglas: ReglaCruce[];
  canCreate: boolean;
};

export function ReglasCrucesOverview({
  reglas,
  canCreate,
}: ReglasCrucesOverviewProps) {
  const fasesUnicas = Array.from(
    new Set(reglas.map((regla) => regla.fase?.nombre?.trim()).filter(Boolean)),
  ) as string[];
  const reglasProgramadas = reglas.filter(
    (regla) => Boolean(regla.fecha) || Boolean(regla.hora),
  ).length;
  const reglasConEstadio = reglas.filter((regla) => Boolean(regla.estadio?.trim())).length;

  const stats = [
    {
      label: "Reglas cargadas",
      detail: "Total real visible",
      value: String(reglas.length),
      icon: Network,
      toneClass: "bg-[#5993B6]/18 text-[#AEEBFF]",
    },
    {
      label: "Fases cubiertas",
      detail: "Con reglas configuradas",
      value: String(fasesUnicas.length),
      icon: ShieldCheck,
      toneClass: "bg-[#FAB438]/14 text-[#FFE4A3]",
    },
    {
      label: "Con horario o fecha",
      detail: "Programacion asignada",
      value: String(reglasProgramadas),
      icon: TimerReset,
      toneClass: "bg-emerald-400/14 text-emerald-200",
    },
  ];

  const reglasDestacadas = reglas.slice(0, 3);
      ...
      <section className={`${DASHBOARD_PANEL} rounded-[32px] p-3 md:p-4`}>
        ...
        <div className="grid w-full min-w-0 gap-4 2xl:grid-cols-[minmax(0,2.05fr)_minmax(300px,0.95fr)] 2xl:items-stretch">
          <section className="relative min-w-0 overflow-hidden rounded-[30px] border border-white/10 bg-[#1E2C46] px-4 py-5 text-white shadow-[0_24px_70px_rgba(2,6,23,0.24)] md:px-6 md:py-6 xl:px-7 xl:py-6 2xl:px-8 2xl:py-7">
              ...
              <div className="flex h-full flex-col">
                ...
                <div className="mt-6 space-y-3 xl:mt-8">
                  <h1 className="text-[2.1rem] font-bold leading-[0.98] tracking-[-0.065em] md:text-[2.35rem] xl:text-[2.55rem] 2xl:text-[2.9rem]">
                    Reglas de <span className="text-[#5993B6]">cruces</span>
                  </h1>
                  ...
                  <p className="max-w-[560px] pt-1 text-[0.95rem] leading-6 text-white/78 xl:text-[1rem]">
                    Aca definis como se arma el cuadro del torneo, con origenes por
                    posicion, fase, estadio y programacion para cada cruce.
                  </p>
                </div>

                <div className="mt-5 flex flex-wrap gap-2">
                  {fasesUnicas.slice(0, 5).map((fase) => (
                    <Badge
                      key={fase}
            ...
            <div className="pointer-events-none absolute bottom-[-12px] right-[-18px] z-20 hidden h-[440px] w-[340px] xl:block 2xl:bottom-[-18px] 2xl:right-[-8px] 2xl:h-[510px] 2xl:w-[390px]">
              ...
              <Image
                src="/brand/cruces_fixture.png"
                alt="Ilustracion para reglas de cruces"
                fill
                priority
      ...
      <section className="grid gap-4">
            ...
            <div className="relative z-10 space-y-6">
              <div>
                ...
                <p className="mt-2 max-w-[760px] text-sm leading-6 text-white/72">
                  Primeras reglas reales para validar nombre del partido, origenes y
                  nivel de programacion actual.
                </p>
              </div>

              <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
                {reglasDestacadas.map((regla) => (
                  <article
                    key={regla.id}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <h3 className="mt-3 font-brand text-[1.55rem] leading-none tracking-[0.04em] text-white">
                          {regla.nombre}
                        </h3>
                      </div>
                    ...
                    <div className="mt-4 space-y-2 text-sm leading-6 text-white/72">
                      ...
                      <p>
                        ...
                        {regla.visitanteOrigen}
                      </p>
                      <p>
                        <span className="font-semibold text-white">Estadio:</span>{" "}
                        {regla.estadio || "Sin definir"}
                      </p>
                    </div>
                  </article>
                ))}
        ...
        <aside className={`${DASHBOARD_PANEL} rounded-[32px] p-4 md:p-5`}>
          ...
          <div className="relative z-10 space-y-4">
            ...
            <div className={`rounded-[22px] p-4 ${DASHBOARD_SUBCARD}`}>
              <p className="text-sm font-semibold text-white">Cobertura de fases</p>
              <p className="mt-2 text-sm leading-6 text-white/72">
                Hoy hay {fasesUnicas.length} fase{fasesUnicas.length === 1 ? "" : "s"}{" "}
                con reglas cargadas y {reglasConEstadio} regla
                {reglasConEstadio === 1 ? "" : "s"} con estadio definido.
              </p>
            </div>

            <div className={`rounded-[22px] p-4 ${DASHBOARD_SUBCARD}`}>
              <p className="text-sm font-semibold text-white">Programacion</p>
              <p className="mt-2 text-sm leading-6 text-white/72">
                {reglasProgramadas} de {reglas.length} regla
                {reglas.length === 1 ? "" : "s"} ya tienen fecha o hora asociada.
              </p>
            </div>
          </div>
        </aside>
