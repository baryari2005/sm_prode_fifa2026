...
export function ReglasCrucesDataTable<T>({
  data,
  loading,
  page,
  totalPages,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-[24px] border border-white/10 bg-[#243754]/55">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => (
                  <th
                    key={header.id}
                    className="px-3 py-3 text-left text-[11px] font-black uppercase tracking-[0.16em] text-[#AEEBFF] whitespace-normal"
                  >
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      key={cell.id}
                      className="px-3 py-3 align-top whitespace-normal break-words"
                    >
